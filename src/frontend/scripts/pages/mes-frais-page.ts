...
export class MesFraisPage {
  ...
  public async render(): Promise<void> {
    ...
    const html = await renderTemplate(
      "/src/frontend/templates/mes-frais-page.tpl.html",
      {}
    );
  ...
  private async renderTable(): Promise<void> {
    ...
    if (this.filteredFrais.length === 0) {
      const buttonHtml = await renderTemplate(
        "/src/frontend/templates/nouveau-frais-button.tpl.html",
        {}
      );
    ...
    const tableHtml = await renderTemplate(
      "/src/frontend/templates/frais-table.tpl.html",
      {}
    );
  ...
  private async renderFraisRow(frais: Frais): Promise<string> {
    ...
    let actionsHtml = await renderTemplate(
      "/src/frontend/templates/frais-action-view.tpl.html",
      { id: frais.id_frais }
    );

    if (frais.statut === "Brouillon") {
      const editDeleteHtml = await renderTemplate(
        "/src/frontend/templates/frais-action-edit-delete.tpl.html",
        { id: frais.id_frais }
      );
      actionsHtml += editDeleteHtml;
    }

    return await renderTemplate("/src/frontend/templates/frais-row.tpl.html", {
      date,
