...
export class TarifsPage {
  ...
  public async render(): Promise<void> {
    ...
    const html = await renderTemplate(
      "/src/frontend/templates/pages/rates-page.tpl.html",
      {}
    );
  ...
  private async renderTable(): Promise<void> {
    ...
    const tableTemplate = await renderTemplate(
      "/src/frontend/templates/components/rates/rates-table.tpl.html",
      { actionsHeaderHtml }
    );
    ...
    if (tbody) {
      if (paginatedTarifs.length === 0) {
        const emptyTemplate = await renderTemplate(
          "/src/frontend/templates/components/rates/rates-empty.tpl.html",
  ...
  private async renderPaginationButtons(totalPages: number): Promise<void> {
    ...
    // Bouton Previous
    const prevButton = await renderTemplate(
      "/src/frontend/templates/common/pagination-nav-button.tpl.html",
      {
        ...
        classes:
          this.currentPage === 1
            ...
            : "hover:bg-accent",
        disabledAttr: this.currentPage === 1 ? "disabled" : "",
      }
    );
    buttons += prevButton;
    ...
    if (totalPages <= 7) {
      for (let i = 1; i <= totalPages; i++) {
        buttons += await this.createPageButton(i);
      }
    } else {
      if (this.currentPage <= 4) {
        ...
        buttons += await this.createPageButton(totalPages);
      } else if (this.currentPage >= totalPages - 3) {
        buttons += await this.createPageButton(1);
        buttons += '<span class="px-2">...</span>';
  ...
  private async createPageButton(page: number): Promise<string> {
    const isActive = page === this.currentPage;
    return await renderTemplate(
      "/src/frontend/templates/common/pagination-page-button.tpl.html",
      {
        page,
        classes: isActive
  ...
  private async renderTarifRow(tarif: GrilleTarifaire): Promise<string> {
    ...
    let actionsCellHtml = "";
    if (this.canEdit) {
      actionsCellHtml = await renderTemplate(
        "/src/frontend/templates/rates/rate-edit-cell.tpl.html",
        { cylindree: tarif.cylindree }
      );
    }

    return await renderTemplate(
      "/src/frontend/templates/components/rates/rate-row.tpl.html",
      {
        cylindree: tarif.cylindree,
