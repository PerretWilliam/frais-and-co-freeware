...
export class VehiculesPage {
  ...
  public async render(): Promise<void> {
    ...
    const html = await renderTemplate(
      "/src/frontend/templates/pages/vehicles-page.tpl.html",
      {}
    );
    ...
    if (this.isEmploye) {
      ...
      if (alertContainer) {
        alertContainer.innerHTML = await renderTemplate(
          "/src/frontend/templates/components/vehicles/vehicle-alert.tpl.html",
          {}
        );
      }
      ...
      if (!hasVehicule) {
        ...
        contentContainer.innerHTML = await renderTemplate(
          "/src/frontend/templates/components/vehicles/vehicles-empty.tpl.html",
          {}
        );
        ...
        contentContainer.innerHTML = await renderTemplate(
          "/src/frontend/templates/components/vehicles/vehicle-card-employee.tpl.html",
          {
      ...
      const cardsHtml = await Promise.all(
        this.allUtilisateurs.map((user) =>
          renderTemplate(
            "/src/frontend/templates/components/vehicles/vehicle-card-admin.tpl.html",
            {
              marque: user.marque,
              modele: user.modele,
              plaque: user.plaque,
      ...
      const gridHtml = await renderTemplate(
        "/src/frontend/templates/vehicles/vehicles-grid.tpl.html",
        { cards: cardsHtml.join("") }
      );
