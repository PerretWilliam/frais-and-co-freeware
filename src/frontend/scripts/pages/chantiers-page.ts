...
export class ChantiersPage {
  ...
  public async render(): Promise<void> {
    ...
    const html = await renderTemplate(
      "/src/frontend/templates/chantiers-page.tpl.html",
      {}
    );
  ...
  private async renderTable(): Promise<void> {
    ...
    if (this.filteredChantiers.length === 0) {
      const buttonHtml = this.canEdit
        ? await renderTemplate(
            "/src/frontend/templates/add-chantier-button.tpl.html",
            {}
          )
        : "";

      container.innerHTML = await renderTemplate(
        "/src/frontend/templates/chantiers-empty.tpl.html",
        {
          message:
    ...
    const tableHtml = await renderTemplate(
      "/src/frontend/templates/chantiers-table.tpl.html",
      {}
    );
    ...
    if (this.canEdit) {
      ...
      if (this.canDelete) {
        deleteHtml = await renderTemplate(
          "/src/frontend/templates/chantier-delete-button.tpl.html",
          { id: chantier.id_chantier }
        );
      }
      editDeleteHtml = await renderTemplate(
        "/src/frontend/templates/chantier-edit-delete.tpl.html",
        { id: chantier.id_chantier, deleteHtml }
      );
    }

    const actionsHtml = await renderTemplate(
      "/src/frontend/templates/chantier-actions.tpl.html",
      { id: chantier.id_chantier, editDeleteHtml }
    );

    return await renderTemplate(
      "/src/frontend/templates/chantier-row.tpl.html",
      {
