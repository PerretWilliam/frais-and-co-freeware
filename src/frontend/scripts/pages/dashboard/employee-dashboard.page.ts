...
export class DashboardEmployePage {
  ...
  public async render(): Promise<void> {
    ...
    const html = await renderTemplate(
      "/src/frontend/templates/pages/dashboard-employe.tpl.html",
      {}
    );
  ...
  private async renderRecentExpenses(frais: Frais[]): Promise<void> {
    ...
    if (frais.length === 0) {
      const emptyHtml = await renderTemplate(
        "/src/frontend/templates/common/empty-state.tpl.html",
        {
    ...
    const expensesHtml = await Promise.all(
      frais.map(async (f) => {
        ...
        return await renderTemplate(
          "/src/frontend/templates/components/expenses/recent-expense-item.tpl.html",
          {
            lieu: f.lieu,
