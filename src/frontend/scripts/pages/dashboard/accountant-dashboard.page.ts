...
export class DashboardComptablePage {
  ...
  public async render(): Promise<void> {
    ...
    const html = await renderTemplate(
      "/src/frontend/templates/pages/dashboard-comptable.tpl.html",
      {}
    );
  ...
  private async showAlert(count: number): Promise<void> {
    ...
    container.innerHTML = await renderTemplate(
      "/src/frontend/templates/common/alert-pending-frais.tpl.html",
      {
        count: count,
      }
    );
    ...
    const items = await Promise.all(
      frais.map(async (f) => {
        ...
        return await renderTemplate(
          "/src/frontend/templates/components/expenses/validation-item.tpl.html",
          {
